...
class Riden:
    ...
    def get_adc_system_temp(self, _adc_system_temp: int = None) -> int:
        self.adc_system_temp = _adc_system_temp or self.read(R.ADC_SYSTEM_TEMP)
        return self.adc_system_temp

    def get_adc_battery_temp(self, _adc_battery_temp: int = None) -> int:
        self.adc_battery_temp = _adc_battery_temp or self.read(R.ADC_BATTERY_TEMP)
        return self.adc_battery_temp

    def get_adc_input_voltage(self, _adc_input_voltage: int = None) -> int:
        self.adc_input_voltage = _adc_input_voltage or self.read(R.ADC_INPUT_VOLTAGE)
        return self.adc_input_voltage

    def get_adc_battery_voltage(self, _adc_battery_voltage: int = None) -> int:
        self.adc_battery_voltage = _adc_battery_voltage or self.read(R.ADC_BATTERY_VOLTAGE)
        return self.adc_battery_voltage

    def get_adc_output_voltage(self, _adc_output_voltage_h: int = None, _adc_output_voltage_l: int = None) -> int:
        _adc_output_voltage_h = _adc_output_voltage_h or self.read(R.ADC_OUTPUT_VOLTAGE_H)
        _adc_output_voltage_l = _adc_output_voltage_l or self.read(R.ADC_OUTPUT_VOLTAGE_L)
        self.adc_output_voltage = (_adc_output_voltage_h << 16) | _adc_output_voltage_l
        return self.adc_output_voltage

    def get_adc_output_current(self, _adc_output_current_h: int = None, _adc_output_current_l: int = None) -> int:
        _adc_output_current_h = _adc_output_current_h or self.read(R.ADC_OUTPUT_CURRENT_H)
        _adc_output_current_l = _adc_output_current_l or self.read(R.ADC_OUTPUT_CURRENT_L)
        self.adc_output_current = (_adc_output_current_h << 16) | _adc_output_current_l
        return self.adc_output_current
    ...
    def update(self) -> None:
        ...
        fw = 1
        if fw >= 1: # Firmware V1.43.1k or higher needed
            # Calculate the number of None placeholders required to bridge the gap
            gap_size = R.ADC_SYSTEM_TEMP - len(data)
            data += (None,) * gap_size
            
            # Fetch and append the new registers
            data += self.read(R.ADC_SYSTEM_TEMP, (R.ADC_OUTPUT_CURRENT_L - R.ADC_SYSTEM_TEMP) + 1)
            
            # Update the device properties using the correct index
            self.get_adc_system_temp(data[R.ADC_SYSTEM_TEMP])
            self.get_adc_battery_temp(data[R.ADC_BATTERY_TEMP])
            self.get_adc_input_voltage(data[R.ADC_INPUT_VOLTAGE])
            self.get_adc_battery_voltage(data[R.ADC_BATTERY_VOLTAGE])
            self.get_adc_output_voltage(data[R.ADC_OUTPUT_VOLTAGE_H], data[R.ADC_OUTPUT_VOLTAGE_L])
            self.get_adc_output_current(data[R.ADC_OUTPUT_CURRENT_H], data[R.ADC_OUTPUT_CURRENT_L])
    ...
    def get_v_bat(self, _v_bat: int = None) -> float:
        ...
        self.v_bat = _v_bat / self.v_bat_multi
        return self.v_bat
